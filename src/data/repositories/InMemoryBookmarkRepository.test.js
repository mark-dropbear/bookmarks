...
describe('InMemoryBookmarkRepository', () => {
  ...
  it('should search bookmarks by tag', async () => {
    const bookmark1 = new Bookmark({ name: 'T1', url: 'https://t1.com', tags: ['tag1'] });
    const bookmark2 = new Bookmark({ name: 'T2', url: 'https://t2.com', tags: ['tag2'] });

    await repository.add(bookmark1);
    await repository.add(bookmark2);

    const results = await repository.search('tag1');
    expect(results).to.have.lengthOf(1);
