...
import { IndexedDBBookmarkRepository } from './IndexedDBBookmarkRepository.js';
...
describe('IndexedDBBookmarkRepository', () => {
  const TEST_DB_NAME = 'bookmarks-repo-test-db';
  let repository;
  let db;

  beforeEach(async () => {
    db = await openIndexedDB(TEST_DB_NAME, 1, { upgrade: upgradeDatabase });
    repository = new IndexedDBBookmarkRepository(db);
  });
